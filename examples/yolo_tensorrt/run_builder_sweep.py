...
def predefined_configurations(workspace_gb: float = REFERENCE_WORKSPACE_GB) -> list[dict[str, Any]]:
    ...
    return [{"label": label, "builder_optimization_level": level,
             ...
             "workspace_gb": 1.0 if label == "workspace1" else workspace_gb,
             "max_aux_streams": aux,
             "max_aux_streams_mode": "auto" if aux == "auto" else "explicit"}
            for label, level, avg, tactics, _workspace, aux in CONFIGURATIONS]
...
def create_summary(configurations: list[dict[str, Any]], builds: dict[str, dict[str, Any]],
    ...
    for config in configurations:
        ...
        rows.append(row)
    reference = next(row for row in rows if row["label"] == "reference")
    for row in rows:
    ...
    winner = rows[0]
    advantage = (reference["compute_median_ms"] - winner["compute_median_ms"]) / reference["compute_median_ms"] * 100
    return {"reference_label": "reference", "winner_label": winner["label"],
            "winner_advantage_over_reference_percent": advantage,
            "winner_advantage_smaller_than_2_percent": advantage < 2.0, "configurations": rows}
...
def main(argv: Sequence[str] | None = None) -> dict[str, Any]:
    ...
    configurations = predefined_configurations(args.workspace_gb); digest = sha256(args.onnx_path)
    started = datetime.now(timezone.utc).isoformat(); builds = {}; commands = []
    for config in configurations:
        ...
        if path.exists() and not args.force:
            ...
            if completed.returncode: raise RuntimeError(f"Build {label} failed with exit code {completed.returncode}; see build_{label}.stderr.log")
    ...
    final_artifacts = (benchmark_json, benchmark_csv, args.output_dir / "builder_sweep_summary.json",
                       args.output_dir / "builder_sweep_summary.csv", args.output_dir / "builder_sweep_manifest.json")
    ...
    for config in configurations: benchmark_args += ["--engine", f'{config["label"]}={engine_path(args.output_dir, config["label"], args.model_precision)}']
    from examples.yolo_benchmark.benchmark_precision import main as benchmark_main
    benchmark = benchmark_main(benchmark_args)
    summary = create_summary(configurations, builds, benchmark)
    ...
    manifest = {"started_at": started, "finished_at": datetime.now(timezone.utc).isoformat(), "onnx_path": str(args.onnx_path),
                "onnx_sha256": digest, "command": [sys.executable, *sys.argv], "build_commands": commands,
                "builder_settings": configurations, "benchmark_settings": {key: getattr(args, key) for key in ("profile", "engine_warmup", "engine_iterations", "engine_rounds", "discard_rounds")},
                "builds": builds, "environment": benchmark.get("environment", {})}
    (args.output_dir / "builder_sweep_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    return summary
