"""Run Stage-4 pairwise selective-FP16 ablations for YOLO INT8.

Stage 4 follows the Stage-3 winner by keeping model.22/other quantized and testing
which two of cv2, cv3, and dfl still need FP16 fallback. Accuracy is measured only;
latency benchmarking remains deferred until the accuracy shortlist is stable.
"""
...
PAIR_VARIANTS: dict[str, tuple[str, str]] = {
    "fp16_cv3_dfl": ("cv3", "dfl"),
    "fp16_cv2_dfl": ("cv2", "dfl"),
    "fp16_cv2_cv3": ("cv2", "cv3"),
}


def pair_variants(groups: dict[str, list[str]]) -> dict[str, list[str]]:
    """Return exact FP16 exclusion sets for the three Stage-4 pairs."""
    required = {"cv2", "cv3", "dfl", "other"}
    if set(groups) != required:
        raise ValueError(f"Stage-4 requires groups {sorted(required)}; found {sorted(groups)}")
    variants: dict[str, list[str]] = {}
    for label, pair in PAIR_VARIANTS.items():
        names = sorted(node for group in pair for node in groups[group])
        if not names:
            raise ValueError(f"Stage-4 variant {label} resolved zero FP16 nodes")
        variants[label] = names
    return variants
...
def main(argv: Sequence[str] | None = None) -> dict[str, Any]:
    ...
    if calibration_count != 128 or calibration_seed != 0:
        raise ValueError(
            "Stage 4 requires entropy_128 calibration metadata with count=128 and seed=0; "
    ...
    manifest: dict[str, Any] = {
        ...
        "stage": "Stage 4 model.22 pairwise FP16 ablation",
        "scope": args.scope,
        "source_fp32_onnx_sha256": source_sha,
        "modelopt_version": modelopt_version,
        "calibration_method": "entropy",
        ...
        "fixed_int8_group": "other",
        "groups": [
            {"name": name, "exact_node_count": len(groups[name]), "exact_node_names": groups[name]}
            for name in ("cv2", "cv3", "dfl", "other")
        ],
        "variants": [],
    }
    ...
    for label, names in variants.items():
        fp16_groups = list(PAIR_VARIANTS[label])
        int8_groups = [group for group in ("cv2", "cv3", "dfl", "other") if group not in fp16_groups]
        ...
        row: dict[str, Any] = {
            ...
            "fp16_group_names": fp16_groups,
            "int8_group_names": int8_groups,
    ...
    summary = {
        ...
        "best_variant": best,
        "winner_note": "The best Stage-4 variant remains an accuracy candidate until a balanced latency benchmark is run.",
        "variants": ordered,
    }
